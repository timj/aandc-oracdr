...
The following are the key design successes of \oracdr:

\begin{itemize}

\item The clean separation of recipes, primitives, instrument
  specification, logging, and algorithm engines. The core design is
  unchanged from the beginning and there are clear benefits to putting
  a single piece of information in a single component. The recipe
  parser rewrite did not affect the pipeline users and neither did the
  reorganization of the class hierarchy for \Frame\ and \Group\
  classes.
...
\item Reusing primitive code for new instruments was a key requirement
  ...
  added. Las Cumbres Observatory are adding additional instruments to
  the pipeline and can reuse the imaging \primitives, only modifying
  code that defines the differences between instruments. A  similar approach
  has been used by the LSST data management stack to allow it to
  support different camera geometries \citep{2010SPIE.7740E..15A}.
...
\item The pipeline was designed and developed in an operations
  environment by people who were going to be supporting it every night
  on two telescopes. This requires robust thinking from the beginning
  to maximize usefulness of output and minimizing support calls in the
  middle of the night. This drove us to design a system with a
