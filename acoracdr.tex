...
\citep[ORAC-DR;][\ascl{1310.001}]{1999ASPC..172...11E,2008AN....329..295C} was
...
\citep[][\ascl{1110.012}]{2014ASPC..485..391C} but this is not
...
\end{lstlisting}
\caption{Code added to the start of each \primitive. In this case the
  \primitive\ is called \texttt{\_TEST\_PRIMITIVE\_}. In debugging
  mode additional lines would be present. Note that the subroutine is
  not named and is simply used to generate an anonymous subroutine
  code reference. The final line in the listing is used to reset the
  line numbering so that error messages can include the correct line
  number when referring to the \primitive\ code as written by the
  programmer. This code has
  been generated with the 2014A version of ORAC-DR.}
\label{fig:primhead}
\end{figure*}
...
read into what is knowns as a \Frame\ object. This object contains all
...
eihter consist of multiple files or which generate multiple
...
frames or parameters derived from calibration instruments. The early
...
In the current system the \Index\ is a file on disk that is cached by
...
mode as the full set of calibrations can be made available. Each
...
are used to form an identifier that is compared with the entries in
...
\citep[][\ascl{1403.024}]{2009ASPC..411..575D} and KAPPA
...
stars in the field to solve the mosaicking offsets). From a software
engineering perspective this was clearly the correct choice but it
...
Archive \citep{2014ASPC..485..143B} reduces entire nights in a single
process but this is less important for UKIRT data as it is uncommon
for a later calibration observation to be usable by an earlier
observation.
...
processes has worked despite it feeling ``kludgey''.
...
the SMURF map-maker \citep[][\ascl{310.007}]{2013MNRAS.430.2545C}, currently supported by
...
Andy Gibb, Paul Hirst,  and Stephen Todd for their significant contributions.
